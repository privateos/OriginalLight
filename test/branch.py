...
cond = graph.placeholder()
t1 = graph.matmul(x1, x2)#(5,4)
t2 = graph.matmul(x1, x3)#(5,4)

def f1(t1, t2):
    return graph.multiply(t1, t2)
def f2(t1, t2):
    return graph.add(t1, t2)

t9 = graph.branch(cond, f1, f2, t1, t2)
y = graph.mean(t9)
...
t_cond0 = tf.placeholder(tf.int32)
t_cond1 = tf.placeholder(tf.int32)
...
def t_f2():
    return t_t1 + t_t2
t_t9 = tf.cond(t_cond0<t_cond1, t_f1, t_f2)
...
with tf.Session() as t_sess:
    ...
    for i in range(epochs):
        ...
        sess_y = sess.run(feed_dict={x1:np_x1, x2:np_x2, x3:np_x3, cond:if_})
        grad_y, grad_x1, grad_x2, grad_x3 = engine.get()

        tf_y = t_sess.run(t_y, feed_dict={t_x1:np_x1, t_x2:np_x2, t_x3:np_x3, t_cond0:m, t_cond1:n})
        t_grad_y, t_grad_x1, t_grad_x2, t_grad_x3 = t_sess.run(t_engine, feed_dict={t_x1:np_x1, t_x2:np_x2, t_x3:np_x3, t_cond0:m, t_cond1:n})
