...
y = graph.sum(graph.sigmoid(graph.add(graph.matmul(x1, x2), graph.matmul(x1, x3))))
...
t_y = tf.reduce_sum(tf.nn.sigmoid(tf.matmul(t_x1, t_x2) + tf.matmul(t_x1, t_x3)))
...
with tf.Session() as t_sess:
    ...
    for i in range(epochs):
        ...
        np_x2 = np.random.uniform(0.05, 0.12, x2_shape)
        np_x3 = np.random.uniform(0.02, 0.2, x3_shape)
