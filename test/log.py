...
y = graph.sum(graph.log(graph.add(graph.matmul(x1, x2), graph.matmul(x1, x3))))
...
t_y = tf.reduce_sum(tf.log(tf.matmul(t_x1, t_x2) + tf.matmul(t_x1, t_x3)))
...
with tf.Session() as t_sess:
    ...
    for i in range(epochs):
        np.random.seed(i)
        np_x1 = np.random.uniform(2.1, 3.5, x1_shape)
        np_x2 = np.random.uniform(3.0, 3.8, x2_shape)
        np_x3 = np.random.uniform(2.9, 4.2, x3_shape)
