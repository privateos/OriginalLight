...
t = graph.tanh(graph.add(graph.matmul(x1, x2), graph.matmul(x1, x3)))
y = graph.sum(graph.reshape(t, (-1, )))
#y = graph.reshape(t, (-1, ))
...
t_y = tf.reduce_sum(tf.reshape(tf.nn.tanh(t), (-1,)))
#t_y = tf.reshape(tf.nn.tanh(t), (-1,))
