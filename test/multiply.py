...
y = graph.multiply(x1, graph.multiply(x2, x3))
...
t_y = t_x1*(t_x2*t_x3)#t_x1 - (t_x2 - t_x3) 
...
with tf.Session() as t_sess:
    ...
    for i in range(epochs):
        np.random.seed(i)
        np_x1 = np.random.randn(*x1_shape)
        np_x2 = np.random.random(x2_shape)
        np_x3 = np.random.random(x3_shape)
