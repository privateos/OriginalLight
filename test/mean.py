...
y = graph.sum(graph.mean(graph.divide(x1, graph.divide(x2, x3)), axis=0))
...
t_y = tf.reduce_sum(tf.reduce_mean(t_x1/(t_x2/t_x3), axis=0))
...
with tf.Session() as t_sess:
    ...
    for i in range(epochs):
        ...
        np_x2 = np.random.uniform(2.0, 3.0, x2_shape)
        np_x3 = np.random.uniform(3.0, 4.0, x3_shape)
