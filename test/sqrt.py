...
y = graph.sqrt(graph.mean(graph.add(graph.sqrt(graph.matmul(x1, x2)), graph.matmul(x1, x3))))
...
t_y = tf.sqrt(tf.reduce_mean(tf.sqrt(tf.matmul(t_x1, t_x2)) + tf.matmul(t_x1, t_x3)))
