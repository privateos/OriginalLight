...
y = graph.sqrt(graph.mean(graph.add(graph.power(graph.matmul(x1, x2), graph.constant(np.array(1.3))), graph.matmul(x1, x3))))
...
t_y = tf.sqrt(tf.reduce_mean(tf.pow(tf.matmul(t_x1, t_x2), 1.3) + tf.matmul(t_x1, t_x3)))
