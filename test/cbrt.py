...
y = graph.sqrt(graph.mean(graph.add(graph.cbrt(graph.matmul(x1, x2)), graph.matmul(x1, x3))))
...
t_y = tf.sqrt(tf.reduce_mean(tf.pow(tf.matmul(t_x1, t_x2), 1.0/3.0) + tf.matmul(t_x1, t_x3)))
