...
import flow as F
from flow.common import compute_lib as np
#import flow.common.compute_lib as np
...
y = graph.sum(graph.abs(graph.add(graph.matmul(x1, x2), graph.matmul(x1, x3))))
sess = graph.session([y])
engine = graph.engine(y, [y, x1, x2, x3])
...
with tf.Session() as t_sess:
    ...
    for i in range(epochs):
        np.random.seed(i)
        np_x1 = np.random.uniform(-0.1, 0.2, x1_shape)
        np_x2 = np.random.uniform(-0.15, 0.22, x2_shape)
        np_x3 = np.random.uniform(-0.18, 0.35, x3_shape)
        sess_y = sess.run(feed_dict={x1:np_x1, x2:np_x2, x3:np_x3})
        grad_y, grad_x1, grad_x2, grad_x3 = engine.gradients()
